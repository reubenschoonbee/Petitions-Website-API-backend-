...
//     const [result] = await getPool().query(query, [[u.email, u.firstName, u.lastName, u.password]])
//     return result;
//
// }
// // const findUserWithEmail = async (emailAddress: string): Promise<user> => {
// //     const query = "SELECT * FROM `user` WHERE `email` = ?"
// //     const [result] = await getPool().query(query, [emailAddress]);
// //     if (result[0].length === 0){
// //         return null;
// //     }
// //     // camelizeKeys(result[0][0])
// //     return result;
// // }
// const findUserByEmail = async(email: string): Promise<user> => {
//     const query = 'SELECT * FROM `user` WHERE `email` = ?';
...
// const login = async (id: number, token: string): Promise<ResultSetHeader> =>{
//     const query = "UPDATE user SET auth_token = ? WHERE id = ?"
//     const [result] = getPool().query(query, [token, id])
//     return result
// }
//
//
//
//
// export {register, findUserByEmail, login}
